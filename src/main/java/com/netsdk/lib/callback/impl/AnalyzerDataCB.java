...
public class AnalyzerDataCB implements NetSDKLib.fAnalyzerDataCallBack {
	...
	public int invoke(NetSDKLib.LLong lAnalyzerHandle, int dwAlarmType, Pointer pAlarmInfo, Pointer pBuffer,
		...
		case EVENT_IVS_FOG_DETECTION: /// < 起雾检测事件
		{
			...
			String Picture = picturePath + "\\" + System.currentTimeMillis() + ".jpg";
			ToolKits.savePicture(pBuffer, 0, dwBufSize, Picture);
			System.out.println(" 起雾检测事件  时间(UTC)：" + msg.UTC + " 通道号:" + msg.nChannelID + "事件组ID" + msg.nGroupID
					+ "事件数据类型" + msg.emEventType + "雾等级" + msg.stuFogInfo.emFogLevel);
			break;
		}
		case EVENT_IVS_ACCESS_CTL: /// < 门禁事件
		{
			...
			String Picture = picturePath + "\\" + System.currentTimeMillis() + ".jpg";
			ToolKits.savePicture(pBuffer, 0, dwBufSize, Picture);
			System.out.println(" 门禁事件  时间(UTC)：" + msg.UTC + " 通道号:" + msg.nChannelID + "事件组ID" + msg.nGroupID
					+ "事件数据类型" + msg.emEventType + "人员温度信息是否有效" + msg.bManTemperature + "人员温度"
					+ msg.stuManTemperatureInfo.fCurrentTemperature + "温度单位"
					+ msg.stuManTemperatureInfo.emTemperatureUnit + "是否超温"
					+ msg.stuManTemperatureInfo.bIsOverTemperature);
			break;
		}
		case EVENT_IVS_AUDIO_ABNORMALDETECTION: // 声音异常检测
		{
			...
			String Picture = picturePath + "\\" + System.currentTimeMillis() + ".jpg";
			ToolKits.savePicture(pBuffer, 0, dwBufSize, Picture);
			System.out.println(" 声音异常检测 时间(UTC)：" + msg.UTC + " 通道号:" + msg.nChannelID + "声音强度" + msg.nDecibel + "声音频率"
					+ msg.nFrequency);
			break;
		}
		case EVENT_IVS_CLIMBDETECTION: // 攀高检测事件
		{
			...
			stuFileInfo = msg.stuFileInfo;
			stPicInfo = msg.stuObject.stPicInfo;
			System.out.println(" 攀高检测事件 时间(UTC)：" + msg.UTC + " 通道号:" + msg.nChannelID + " 开始时间:"
					+ msg.stuObject.stuStartTime + " 结束时间:" + msg.stuObject.stuEndTime);
			break;
		}
...
//             	PrintStruct.print(msg);
		...
		case EVENT_IVS_STAY_ALONE_DETECTION: /// < 单人独处事件
		{
			...
			ToolKits.savePicture(pBuffer, 0, dwBufSize, Picture);
			System.out.println(
					" 单人独处事件  时间(UTC)：" + msg.UTC + " 通道号:" + msg.nChannelID + "物体ID" + msg.stuHuman.nObjectID);
			break;
		}
		...
		case EVENT_IVS_SHOP_SIGN_ABNORMAL: {// 店招异常事件
			...
			ToolKits.GetPointerData(pAlarmInfo, info);
			stuFileInfo = info.stuFileInfo;
			System.out.println(" 店招异常事件  时间(UTC)：" + info.UTC + " 通道号:" + info.nChannelID + " 商铺地址: "
					+ new String(info.szShopAddress, Charset.forName(Utils.getPlatformEncode())).trim() + " 违法持续时间: "
					+ info.nViolationDuration + " 检测到的物体数量: " + info.nObjectNum);
		...
		case EVENT_IVS_TRAFFICJUNCTION: {
			...
			System.out.println(date + " traffic number:" + new String(trafficCar.szPlateNumber, Charset.forName("GBK"))
					+ ",plate color:" + new String(trafficCar.szPlateColor, Charset.forName("GBK")));
			// save the picture
