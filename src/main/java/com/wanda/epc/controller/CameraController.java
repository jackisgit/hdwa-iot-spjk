...
import com.wanda.epc.pojo.HikvisionCameraPojo;
import com.wanda.epc.pojo.HikvisionDevicePojo;
import com.wanda.epc.pojo.HikvisionPlayBackLisPojo;
...
public class CameraController {
        ...
        if (!CollectionUtils.isEmpty(eqIds)) {
            List<HikvisionDevicePojo> hikvisionDeviceDtoList = eqIds.stream().map(eqId -> {
                ...
                if (Objects.nonNull(deviceInfo)) {
                    ...
                    cameraPojo.setChannel(deviceInfo.getIndexCode());
    ...
    public ResultUtil<List<HikvisionPlayBackLisPojo>> historyList(@RequestBody HikvisionPlaybackRequest request) throws InterruptedException {
        ...
        if (Objects.nonNull(deviceInfo)) {
            ...
            cameraPojo.setStarttime(request.getBeginTime());
            cameraPojo.setEndtime(request.getEndTime());
            List<HikvisionPlayBackLisPojo> hikvisionPlayBackListDtos = historyList(cameraPojo);
            return ResultUtil.success(hikvisionPlayBackListDtos);
        }
        return null;
    }
    ...
    public List<HikvisionPlayBackLisPojo> historyList(CameraPojo pojo) throws InterruptedException {
        ...
        pFindCond.read();
        pFindCond.uSeniorParam.struStreamIDParam.struIDInfo.byID = pojo.getChannel().getBytes();
        ...
        HCNetSDK.NET_DVR_TIME_SEARCH_COND lpStartTime = Utils.getCvrTime(pojo.getStarttime());
        HCNetSDK.NET_DVR_TIME_SEARCH_COND lpStopTime = Utils.getCvrTime(pojo.getEndtime());
        ...
        // 文件查找结果信息结构体。
        HCNetSDK.NET_DVR_SEARCH_EVENT_RET_V50 lpFindData = new HCNetSDK.NET_DVR_SEARCH_EVENT_RET_V50();
        ...
        while (true) {
            // 逐个获取查找到的文件信息
            lFindNextFile_V40 = HCNetSDK.INSTANCE.NET_DVR_FindNextEvent_V50(lFindFileHandle, lpFindData);
            // 正在查找请等待
            ...
            if (lFindNextFile_V40 == 1000) {
                HikvisionPlayBackLisPojo hikvisionPlayBackLisPojo = new HikvisionPlayBackLisPojo();
                hikvisionPlayBackLisPojo.setBeginTime(Utils.cvrTimeToStr(lpFindData.struStartTime));
                hikvisionPlayBackLisPojo.setEndTime(Utils.cvrTimeToStr(lpFindData.struEndTime));
                list.add(hikvisionPlayBackLisPojo);
                videoindex++;
                continue;
            }
