...
public class Utils {
    ...
    public static void sdkRelease(NetSDKLib.LLong playHandle, int playSign) {
        logger.info("----实时预览句柄----：{}", playHandle.longValue());
        if (playSign == 0) {
            if (!NetSDKLib.NETSDK_INSTANCE.CLIENT_StopRealPlayEx(playHandle)) {// 停止预览
                logger.error("停止预览 error,code:" + ToolKits.getErrorCode());
            }
        } else {
            if (!NetSDKLib.NETSDK_INSTANCE.CLIENT_StopPlayBack(playHandle)) {// 停止回放
                logger.error("停止回放 error,code:" + ToolKits.getErrorCode());
            }
        }
    }
