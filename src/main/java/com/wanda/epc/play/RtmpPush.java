...
//                logger.info("pkt: {}---- pkt.size:{}--- pkt.data:{}", pkt, pkt.size(), pkt.data());
                if (pkt == null || pkt.size() <= 0 || pkt.data() == null) {
                    if (playSign == 0) {
                        // 空包记录次数跳过
                        logger.warn("JavaCV 出现空包 设备信息：[ip:" + pojo.getIp() + " port:" + pojo.getPort() + " channel:"
                                + pojo.getChannel()  + " starttime:"
            ...
            logger.info("-------推流出现错误----：{}", e.getMessage());
            e.printStackTrace();
            release();
            logger.error(e.getMessage());
        } finally {
            logger.info("-------推流出现错误exitcode----：{}", exitcode);
            release();
            if (exitcode == 2) {
