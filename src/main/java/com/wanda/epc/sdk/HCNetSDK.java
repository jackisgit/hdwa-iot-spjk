...
public interface HCNetSDK extends StdCallLibrary {
    ...
    // windows环境下加载dll动态库文件,如果加载报错，请使用绝对路径执行dll文件夹
    HCNetSDK INSTANCE = (HCNetSDK) Native.loadLibrary("sdk\\hcsdk\\HCNetSDK",
            HCNetSDK.class);
    // linux环境下加载so动态库
//	HCNetSDK INSTANCE = (HCNetSDK) Native.loadLibrary(LIBPath.config.getLibpath() + "/libhcnetsdk.so", HCNetSDK.class);

    // linux环境下加载组件
    public static void tryLoad() {
        // 设置HCNetSDKCom组件库所在路径
        String strPathCom = LIBPath.config.getLibpath();
        ...
        String strPathCrypto = LIBPath.config.getLibpath() + "/libcrypto.so";
        System.arraycopy(strPathCrypto.getBytes(), 0, ptrByteArrayCrypto.byValue, 0, strPathCrypto.length());
        ptrByteArrayCrypto.write();
        HCNetSDK.INSTANCE.NET_DVR_SetSDKInitCfg(3, ptrByteArrayCrypto.getPointer());

        // 设置libssl.so所在路径
        BYTE_ARRAY ptrByteArraySsl = new BYTE_ARRAY(256);
        String strPathSsl = LIBPath.config.getLibpath() + "/libssl.so";
        System.arraycopy(strPathSsl.getBytes(), 0, ptrByteArraySsl.byValue, 0, strPathSsl.length());
        ptrByteArraySsl.write();
        HCNetSDK.INSTANCE.NET_DVR_SetSDKInitCfg(4, ptrByteArraySsl.getPointer());
    }
    ...
    public static final int NET_DVR_VOD_DRAG_ING = 37;
    public static final int NET_DVR_VOD_DRAG_END = 38;
    public static final int NET_DVR_PLAYSETTIME = 26;
    public static final int NET_DVR_PLAYSETTIME_V50 = 28;
    public static final int NET_DVR_START_DRAWFRAME = 34;
    //远程按键定义如下：
...
//校时结构参数
    ...
    public static class NET_DVR_COMPRESSION_INFO_V30 extends Structure {
        ...
        public byte byVideoEncType; // 视频编码类型：0-私有264，1-标准h264，2-标准mpeg4，7-M-JPEG，8-MPEG2，9-SVAC，10-标准h265，0xfe-
        // 自动（和源一致），0xff-无效
        public byte byAudioEncType; // 音频编码类型：0-G722，1-G711_U，2-G711_A，5-MP2L2，6-G726，7-AAC，8-PCM，0xfe-
        // 自动（和源一致），0xff-无效
        ...
        public byte bySmartCodec; // 是否启用高性能编码（Smart264）：0- 关闭，1-
        // 打开，启用该功能后码率上限（dwVideoBitrate）不支持修改，平均码率（wAverageVideoBitrate）生效
    ...
    public static class NET_DVR_PTZCFG extends Structure {
        ...
        public NET_DVR_PTZ_PROTOCOL[] struPtz = (NET_DVR_PTZ_PROTOCOL[]) new NET_DVR_PTZ_PROTOCOL()
                .toArray(PTZ_PROTOCOL_NUM);/* 最大200中PTZ协议 */
...
//IPC接入参数配置
    ...
    public static class NET_DVR_IPALARMOUTCFG extends Structure {/* IP报警输出配置结构 */
        ...
        public NET_DVR_IPALARMOUTINFO[] struIPAlarmOutInfo = (NET_DVR_IPALARMOUTINFO[]) new NET_DVR_IPALARMOUTINFO()
                .toArray(MAX_IP_ALARMOUT);/* IP报警输出 */
    }
    ...
    public static class NET_DVR_HDGROUP_CFG extends Structure {
        ...
        public NET_DVR_SINGLE_HDGROUP[] struHDGroupAttr = (NET_DVR_SINGLE_HDGROUP[]) new NET_DVR_SINGLE_HDGROUP()
                .toArray(MAX_HD_GROUP);// 硬盘相关操作都需要重启才能生效；
    }
...
//DS-6001D Decoder
    ...
    public static class NET_DVR_PORTCFG extends Structure {
        ...
        public NET_DVR_PORTINFO[] struTransPortInfo = (NET_DVR_PORTINFO[]) new NET_DVR_PORTINFO()
                .toArray(MAX_TRANSPARENTNUM); /* 数组0表示232 数组1表示485 */
    }
    ...
    public static class NET_DVR_SHOWSTRING_V30 extends Structure {
        ...
        public NET_DVR_SHOWSTRINGINFO[] struStringInfo = (NET_DVR_SHOWSTRINGINFO[]) new NET_DVR_SHOWSTRINGINFO()
                .toArray(MAX_STRINGNUM_V30); /* 要显示的字符内容 */
    }
    ...
    public static class NET_DVR_SHOWSTRING_EX extends Structure {
        public int dwSize;
        public NET_DVR_SHOWSTRINGINFO[] struStringInfo = (NET_DVR_SHOWSTRINGINFO[]) new NET_DVR_SHOWSTRINGINFO()
                .toArray(MAX_STRINGNUM_EX); /* 要显示的字符内容 */
    }
    ...
    public static class NET_DVR_SHOWSTRING extends Structure {
        public int dwSize;
        public NET_DVR_SHOWSTRINGINFO[] struStringInfo = (NET_DVR_SHOWSTRINGINFO[]) new NET_DVR_SHOWSTRINGINFO()
                .toArray(MAX_STRINGNUM); /* 要显示的字符内容 */
    }
...
//矩阵行信息 2007-12-28
    ...
    public static class NET_DVR_MATRIX_TRAN_CHAN_CONFIG extends Structure {
        ...
        public NET_DVR_MATRIX_TRAN_CHAN_INFO[] struTranInfo = (NET_DVR_MATRIX_TRAN_CHAN_INFO[]) new NET_DVR_MATRIX_TRAN_CHAN_INFO()
                .toArray(MAX_SERIAL_NUM);/* 同时支持建立MAX_SERIAL_NUM个透明通道 */
    }
    ...
    public static class NET_DVR_IPPARACFG_V40 extends Structure {
        ...
        public NET_DVR_IPDEVINFO_V31[] struIPDevInfo = (NET_DVR_IPDEVINFO_V31[]) new NET_DVR_IPDEVINFO_V31()
                .toArray(MAX_IP_DEVICE_V40); /* IP设备 */
...
//}
    ...
    public static class NET_DVR_FILECOND_V50 extends Structure {
        public NET_DVR_STREAM_INFO struStreamID = new NET_DVR_STREAM_INFO(); //流ID或通道号
    ...
    public static class EVENT_STREAMIDRET_V50 extends Structure {
        ...
        public byte[] byTapeIndex = new byte[32];  //文件所在磁带编号
        public byte[] byFileNameEx = new byte[256/*256*/]; //文件名扩展
    ...
    public static class NET_DVR_VOD_PARA_V50 extends Structure {
        public int dwSize;
        public NET_DVR_STREAM_INFO struIDInfo = new NET_DVR_STREAM_INFO();
        public NET_DVR_TIME_V50 struBeginTime = new NET_DVR_TIME_V50();
        public NET_DVR_TIME_V50 struEndTime = new NET_DVR_TIME_V50();
        public HWND hWnd;
        public byte byDrawFrame;
        public byte byVolumeType;
        public byte byVolumeNum;
...
//触发参数
    ...
    public static class NET_DVR_VIDEOEFFECT extends Structure {
        ...
        public byte byEnableFunc; // 使能，按位表示。bit0-SMART
        // IR(防过曝)，bit1-低照度，bit2-强光抑制使能，值：0-否，1-是，例如byEnableFunc&0x2==1表示使能低照度功能；
        // bit3-锐度类型，值：0-自动，1-手动
    ...
    public static class NET_DVR_WHITEBALANCE extends Structure {
        public byte byWhiteBalanceMode; // 0-手动白平衡（MWB），1-自动白平衡1（AWB1，范围小），2-自动白平衡2（AWB2，范围宽，2200K-15000K），3-
        // 锁定白平衡（Locked WB），4-室外，5-室内，6-日光灯，7-钠灯，8-自动跟踪（Auto-Track），9-一次白平衡（One
        // Push），10-室外自动（Auto-Outdoor），11-钠灯自动(Auto-Sodiumlight)，12-水银灯模式(Mercury
        // Lamp)，13-自动白平衡(Auto)，14-白炽灯 (IncandescentLamp)，15-暖光灯(Warm Light
        // Lamp)，16-自然光(Natural Light)
